...
- 长截图：手动滚动目标窗口，pngshot 连续采样并拼接垂直内容，自动识别固定在视口边缘的导航栏、底栏和侧栏，并平滑半透明窗口后的固定背景接缝。
...
长截图是半自动流程：Wayland 不允许普通应用直接合成滚动事件，因此由用户滚动目标窗口，pngshot 负责连续采样、自动去重和拼接。

```text
第一次按 Mod+Shift+Print  选择范围并开始采集
再次按 Mod+Shift+Print    完成并生成长图
Enter                     面板获得焦点时完成
Esc                       面板获得焦点时取消
```
...
- 只做垂直滚动，保持选区宽度不变。
- 吸顶导航栏、吸底栏和固定侧栏会通过至少三次滚动变化自动识别：匹配时屏蔽，最终只保留一次。
- 可以连续滚动；短暂匹配失败会自动参考最近多帧恢复，不再要求立即回滚。
- 视频、加载动画、闪烁光标等局部动态区域会由二级匹配自动避开；像素护栏会阻止照片或无关画面误拼。
- 点击“完成”后会用有界无损关键帧做一次全局路径校正，尽量修复漏桥接、重复段和偶发位移；无法验证完整路径时保留在线结果并给出 warning。
- 透明终端、编辑器等界面会在全局重建时羽化已对齐的重叠区，减轻固定墙纸随拼接段跳变的横向接缝；大幅变化的动画像素保留完整单帧状态，不做残影式平均。
- 面板显示“请慢一些”时减慢滚动即可，采集仍会继续尝试接回画面。
- 选区外侧的蓝色边界用于提示采集范围，不会进入最终图片。
- 点击“完成”时会等待正在进行的最后一次抓帧，避免长图少掉末尾一段。
...
右键菜单包含区域截图、长截图、钉图、截图后保存、截图后复制、运行诊断、重启服务和退出托盘。采集长截图时，“长截图”菜单项会变为“完成长截图”。
